...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player #removed +1 from column by muhammad bagosher
    return game
...
def switch_player(player):
    if player == 1: #added = by muhammad bagosher
        return 2
    else:
        return 1
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        available = False
        while not available: #added : at the end of while statement by muhammad bagosher
            row = convert_input_to_coordinate(int(input("Which row? (start with 1) ")))
            column = convert_input_to_coordinate(int(input("Which column? (start with 1) ")))
            available = check_space_empty(game, row, column) #added 3rd positional argument column by muhammad bagosher
        game = add_piece(game, player, row, column)
        display_game(game)
        player = switch_player(player)
        winner = check_winner(game) #removed # before winner by muhammad bagosher
